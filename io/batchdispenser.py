...
class FeatureReader:
	...
	#	scpfile: the scp filename
	#	cmvnfile: the cmvn filename
	#	utt2spkfile:file containing the mapping from utterance ID to speaker ID
	...
	#returns spliced and normalized features
	def getUtt(self):
		#read utterance
...
class Batchdispenser:
	#create a batchdispenser object
	#	featureReader: a feature reader object
	#	size: the batch size
	#	scpfile: the scp filename
	#	alifile: the alignment filename
	...
	#return a batch of features and alignments in one-hot encoding 	
	def getBatch(self):
