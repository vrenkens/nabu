...
from nabu.neuralnetworks.components.attention import LocationAwareAttention
...
class Speller(rnn_decoder.RNNDecoder):
    ...
    def create_cell(self, encoded, encoded_seq_length, is_training):
        ...
        if encoded is not None:
            ...
            attention_mechanisms = [LocationAwareAttention(
                num_units=rnn_cell.output_size,
                numfilt=int(self.conf['numfilt']),
                filtersize=int(self.conf['filtersize']),
                memory=encoded[e],
...
def normalized_sigmoid(x, axis=-1):
    ...
    return sig/tf.reduce_sum(x, axis, keep_dims=True)
