...
class Trainer(object):
    ...
    def __init__(self, classifier, input_dim, init_learning_rate,
                 learning_rate_decay, num_steps, numframes_per_batch):
        '''
        ...
        Args:
            ...
            numframes_per_batch: determines how many frames are processed at a
        ...
        with self.graph.as_default():
            ...
            self.targets = tf.placeholder(
                tf.float32, shape=[None, classifier.output_dim], name='targets')

            #input for the total number of frames that are used in the batch
            self.num_frames = tf.placeholder(tf.float32, shape=[],
                                             name='num_frames')

            #compute the training outputs of the nnetgraph
            trainlogits, self.modelsaver, self.control_ops = classifier(
                self.inputs, is_training=True, reuse=False, scope='Classifier')

            #compute the validation output of the nnetgraph
            logits, _, _ = classifier(self.inputs, is_training=False,
            ...
            with tf.name_scope('train'):
                ...
                loss = tf.reduce_sum(self.compute_loss(self.targets,
                                                       trainlogits))
                ...
                self.update_gradients_op = tf.group(*(
                    ...
                    + [self.update_loss] + update_ops), name='update_gradients')

                #create an operation to apply the gradients
                meangrads = [tf.div(grad, self.num_frames, name=grad.op.name)
                             for grad in grads]
            ...
            with tf.name_scope('valid'):
                ...
                valid_loss = tf.reduce_sum(self.compute_loss(self.targets,
                                                             logits))

                #operation to update the validation loss
                self.update_valid_loss = batch_loss.assign(batch_loss
                                                           + valid_loss).op
    ...
    def compute_loss(self, targets, logits): #pylint: disable=R0201
        ...
        Args:
            targets: a NxO tensor containing the reference targets where N is
                the number of frames and O is the neural net output dimension
            logits: a NxO tensor containing the neural network output logits
                where N is the number of frames and O is the neural net output
                dimension

        Returns:
            an N-dimensional tensor containing the losses for all the input
            frames where N is the number of frames
        '''
    ...
    def update(self, inputs, targets):
        ...
        Args:
            inputs: the inputs to the neural net, this should be a NxF numpy
                array where N is the number of frames in the batch and F is the
                feature dimension
            targets: the one-hot encoded targets for neural nnet, this should be
                an NxO matrix where O is the output dimension of the neural net
        ...
        if (self.numframes_per_batch == -1
                or self.numframes_per_batch > inputs.shape[0]):
        ...
        for k in range(
                int(inputs.shape[0]/numframes_per_batch)
                + int(inputs.shape[0]%numframes_per_batch > 0)):

            batch_inputs = inputs[
                k*numframes_per_batch:
                min((k+1)*numframes_per_batch, inputs.shape[0]), :]
            ...
            self.update_gradients_op.run(feed_dict={self.inputs:batch_inputs,
        ...
        if self.summarywriter is not None:
            [loss, summary, _] = tf.get_default_session().run(
                ...
                feed_dict={self.num_frames:inputs.shape[0]})
