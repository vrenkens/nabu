	...
	if found_segments:
		for utt in segments:
			...
			for seg in segments[utt]:
				if feat_type == 'fbank':
					features = feat.compute_fbank(utterance[int(seg[1]*rate):int(seg[2]*rate)], rate, float(conf['winlen']), float(conf['winstep']), int(conf['nfilt']), int(conf['nfft']), float(conf['lowfreq']), highfreq, float(conf['preemph']), conf['include_energy'] == 'True', conf['snip_edges'] == 'True')
				elif feat_type == 'mfcc':
		...
		for utt in wavfiles:
			if wavfiles[utt][1]:
				...
				(rate,utterance) = wav.read(wavfiles[utt][0])
			if feat_type == 'fbank':
				features = feat.compute_fbank(utterance, rate, float(conf['winlen']), float(conf['winstep']), int(conf['nfilt']), int(conf['nfft']), float(conf['lowfreq']), highfreq, float(conf['preemph']), conf['include_energy'] == 'True', conf['snip_edges'] == 'True')
			elif feat_type == 'mfcc':
